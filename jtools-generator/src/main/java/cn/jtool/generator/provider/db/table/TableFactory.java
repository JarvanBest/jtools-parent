package cn.jtool.generator.provider.db.table;
...
import cn.jtool.generator.util.*;
import cn.jtool.generator.util.XMLHelper;
import cn.jtool.generator.util.XMLHelper.NodeData;
...
public class TableFactory {
	...
	private Table createTable(Connection conn, ResultSet rs) throws SQLException {
		...
		try {
			...
			else if(StringHelper.isBlank(remarks)  && dbHelper.isMysqlDataBase()) {
				String sql = "show table status WHERE NAME='"+realTableName+"'";
				remarks = dbHelper.queryForStringByField(sql,"Comment") ;
			}
   ...
   private String getDatabaseStructureInfo() {
	      ...
	      // Let's give the basic some feedback. The exception
	...
	private List getTableColumns(Table table, List primaryKeys, List indices, Map uniqueIndices, Map uniqueColumns) throws SQLException {
		// get the columns
	      ...
	      while (columnRs.next()) {
	         ...
	         if(StringHelper.isBlank(remarks)  && dbHelper.isOracleDataBase()) {
	        	 remarks = getOracleColumnComments(table.getSqlName(), columnName);
	         }else if(StringHelper.isBlank(remarks) &&dbHelper.isMSSQLDataBase()){
	        	 String sql="SELECT value FROM sys.extended_properties WHERE major_id = OBJECT_ID('"+table.getSqlName()+"') AND minor_id = COLUMNPROPERTY(major_id, '"+columnName+"', 'ColumnId')";
	        	 remarks= dbHelper.queryForString(sql);
	         }
	...
	class DbHelper {
		...
		public boolean isMSSQLDataBase() {
			boolean ret = false;
			try {
				ret = (getMetaData().getDatabaseProductName().toLowerCase()
						.indexOf("microsoft") != -1);
		...
		public String queryForStringByField(String sql, String fieldName) {
			Statement s = null;
			ResultSet rs = null;
			try {
				s =  getConnection().createStatement();
