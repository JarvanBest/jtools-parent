package cn.jtool.generator.provider.db.sql;


import cn.jtool.generator.provider.db.sql.model.Sql;
import cn.jtool.generator.provider.db.sql.model.SqlParameter;
...
import cn.jtool.generator.util.sqlparse.NamedParameterUtils;
import cn.jtool.generator.util.sqlparse.ParsedSql;
import cn.jtool.generator.util.sqlparse.ResultSetMetaDataHolder;
import cn.jtool.generator.util.sqlparse.SqlParseHelper;
	...
	public class SqlParametersParser {
		...
		private LinkedHashSet<SqlParameter> parseForSqlParameters(ParsedSql parsedSql, Sql sql) throws Exception {
			...
			for(int i = 0; i < parsedSql.getParameterNames().size(); i++) {
				...
				if(isMatchListParam(sql.getSourceSql(), paramName)) { //  只考虑(:username)未考虑(#inUsernames#) and (#{inPassword}),并且可以使用 #inUsername[]#
		...
		public boolean isMatchListParam(String sql, String paramName) {
			return 
			    ...
			    || sql.matches(".*[#$]"+paramName+"\\[]\\.?\\w*[#$].*"); //match #basic[]# $basic[]$ #basic[].age#
		}
