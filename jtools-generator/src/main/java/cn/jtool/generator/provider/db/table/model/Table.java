...
public class Table {
    ...
    public LinkedHashSet<Column> getColumnsWithCreateId() {
        LinkedHashSet<Column> result = new LinkedHashSet<>();
        LinkedHashSet<Column> list = getColumns();
    ...
    public boolean getHasCreateId() {
        LinkedHashSet<Column> list = getColumns();
        for (Column column : list) {
            String name = column.getCamelName().toLowerCase();
            if (name.startsWith("createtime") || name.startsWith("createuserid") ||
                    name.startsWith("lastupdatetime") || name.startsWith("lastupdateuserid")) {
                return true;
            }
        }
        return false;
    ...
    public String getDefaultPkColumn() {
        if (getPkColumns().isEmpty()) {
            return null;
        }
        return getPkColumns().get(0).getSqlName();
    }
    ...
    public String getEntityBaseName() {
        LinkedHashSet<Column> columnList = getColumns();
        Set<String> columMap = new LinkedHashSet<>();
        for (Column column : columnList) {
            columMap.add(column.getSqlName());
        }
        if (columMap.contains("historyid")) {
            return "AbstractFlowEntity,LogicDeleteEntity";
        }
        StringBuilder builder = new StringBuilder();
        builder.append("EntityBase<String>");
        if (columMap.contains("create_time")
                && columMap.contains("update_time")) {
            builder.append(",BussinessEntityBase");
        }
        if (columMap.contains("del_flag")) {
            builder.append(",LogicDeleteEntity");
        }
        return builder.toString();
    }

    public boolean getHasFLowEntity() {
        LinkedHashSet<Column> columnList = getColumns();
        Set<String> columMap = new LinkedHashSet<>();
        for (Column column : columnList) {
            columMap.add(column.getSqlName());
    ...
    public Boolean getHasCreateAndUpdateTimeColumn() {
        LinkedHashSet<Column> columnList = getColumns();
        Set<String> columMap = new LinkedHashSet<>();
        for (Column column : columnList) {
            columMap.add(column.getSqlName());
