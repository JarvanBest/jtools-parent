package cn.jtool.generator.provider.db.sql.model;
...
public class Sql {
	 ...
	 * 示例:
	 * <pre>
	 * select count(*) from basic, 返回值为: Long
	 * select * from basic 返回值为: User
	...
	//  columnsSize大于二并且不是在同一张表中,将创建一个QueryResultClassName类,同一张表中也要考虑创建类
	 ...
	 * 示例:
	 * <pre>
	 * SQL : select * from user_info where username=:basic and password=:pwd limit :offset,:limit
	 * params: basic,pwd,offset,limit
	 * </pre>
	 * @return
	 */
...
//			}
//		});
//		for(SqlParameter s : sortedParams){ //  现在只实现了:username参数替换
...
//	}
	 ...
	 * 示例:
	 * <pre>
	 * sourceSql: select * from basic where username=:username and password=:password
	 * executeSql: select * from basic where username=? and password=?
	 * </pre>
	 * @return
	 */
