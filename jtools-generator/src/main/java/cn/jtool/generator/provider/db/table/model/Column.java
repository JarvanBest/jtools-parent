package cn.jtool.generator.provider.db.table.model;


import cn.jtool.generator.GeneratorProperties;
import cn.jtool.generator.provider.db.table.model.ForeignKey.ReferenceKey;
import cn.jtool.generator.provider.db.table.model.util.ColumnHelper;
import cn.jtool.generator.util.GLogger;
import cn.jtool.generator.util.StringHelper;
import cn.jtool.generator.util.TestDataGenerator;
import cn.jtool.generator.util.typemapping.ActionScriptDataTypesUtils;
import cn.jtool.generator.util.typemapping.DatabaseDataTypesUtils;
import cn.jtool.generator.util.typemapping.JavaPrimitiveTypeMapping;
import cn.jtool.generator.util.typemapping.JdbcType;
...
//			_table.getExportedKeys().addForeignKey(refTable.getSqlName(), hasMany.columnSqlName, getSqlName(), hasMany.columnSqlName.toLowerCase().hashCode());
	...
	private void initOtherProperties() {
		...
		columnName=StringHelper.makeAllWordFirstLetterUpperCase(getSqlName());
...
//	private String rapidValidation;
    ...
    public String getCamelName() {
        //return StringHelper.uncapitalize(getColumnName());
        return StringHelper.camelName(getSqlName());
    }

	public String getFirstUperCamelName(){
		String s =StringHelper.firstUperCamelName(getSqlName());
		return s;
	}

	public Boolean getHasOverride(){
		String sqlName=getSqlName();
		if("del_flag".equals(sqlName)){
			return true;
		}
		if("id".equals(sqlName)){
			return true;
		}
		if("historyid".equals(sqlName)){
			return true;
		}
		if("processinstanceid".equals(sqlName)){
			return true;
		}
		if("processinstance_end_date".equals(sqlName)){
			return true;
		}
		if("processinstancecreatedate".equals(sqlName)){
			return true;
		}
		if("create_time".equals(sqlName) || "update_time".equals(sqlName) ||
		"create_user".equals(sqlName) || "update_user".equals(sqlName)
		){
			return getTable().getHasCreateAndUpdateTimeColumn();
		}
		return false;
	}
