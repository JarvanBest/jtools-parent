...
import cn.jtool.generator.util.PropertiesHelper;
import cn.jtool.generator.util.PropertyPlaceholderHelper;
...
public class GeneratorProperties {
	...
	public static void reload(String... paths) {
		try {
		    String[] configPath= paths;
		    if(configPath==null|| configPath.length==0)
            {
                configPath= PROPERTIES_FILE_NAMES;
            }
			GLogger.println("Start Load GeneratorPropeties from classpath:"+ Arrays.toString(configPath));
			Properties p = new Properties();
			String[] loadedFiles = PropertiesHelper.loadAllPropertiesFromClassLoader(p,configPath);
