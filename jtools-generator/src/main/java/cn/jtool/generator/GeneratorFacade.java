...
import cn.jtool.generator.Generator.GeneratorModel;
import cn.jtool.generator.provider.db.sql.model.Sql;
import cn.jtool.generator.provider.db.table.TableFactory;
import cn.jtool.generator.provider.db.table.model.Table;
import cn.jtool.generator.provider.java.model.JavaClass;
import cn.jtool.generator.util.BeanHelper;
import cn.jtool.generator.util.GLogger;
import cn.jtool.generator.util.GeneratorException;
import cn.jtool.generator.util.SystemHelper;
import cn.jtool.generator.util.typemapping.DatabaseTypeUtils;
...
public class GeneratorFacade {
	...
	public GeneratorFacade(){
		g.setOutRootDir(SystemHelper.getOutRootDir());
	}
    ...
    private Generator getGenerator(String templateRootDir) {
        g.setTemplateRootDir(new File(templateRootDir));
        return g;
    }
    ...
    public class ProcessUtils {
        ...
        public void processByTable(String tableName, String templateRootDir, boolean isDelete) throws Exception {
			...
			if(tableName.endsWith("*")){
				processBySearchTable(templateRootDir,tableName.substring(0,tableName.length()-1),false);
				return;
			}
    		Generator g = getGenerator(templateRootDir);
		...
		public void processBySearchTable(String templateRootDir, String compaireTablename, boolean isDelete) throws Exception {
			List<Table> tables = TableFactory.getInstance().getAllTables();
			List exceptions = new ArrayList();
			for(int i = 0; i < tables.size(); i++ ) {
				try {
					Table table =tables.get(i);
					if(table.getSqlName().toLowerCase().startsWith(compaireTablename.toLowerCase()))
					{
	...
	public static class GeneratorModelUtils {
		...
		public static GeneratorModel newFromTable(Table table) {
			...
			String basepackage = GeneratorProperties.getProperty("basepackage");
			String rootDomainfloder= basepackage.substring(basepackage.lastIndexOf(".")+1);
			Map templateModel = new HashMap();
			templateModel.put("table", table);
			String classLowerName= table.getClassName().toLowerCase();
			String classLowerNameWithoutPrefix=classLowerName.replace(rootDomainfloder,"");
			templateModel.put("classNameLowerCase2", classLowerName);
			templateModel.put("classLowerNameWithoutPrefix", classLowerNameWithoutPrefix);
			...
			filePathModel.put("classNameLowerCase2", classLowerName);
			filePathModel.put("classLowerNameWithoutPrefix", classLowerNameWithoutPrefix);
		...
		public static void setShareVars(Map templateModel) {
			...
			templateModel.put("rootDomainfloder", basepackage.substring(basepackage.lastIndexOf(".")+1));
